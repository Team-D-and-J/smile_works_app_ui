...
const TreatmentHistory: React.FC<TreatmentHistoryProps> = ({ patientId }) => {
  const {
    ...
    isLoading: treatmentListLoading,
  } = useGetTreatmentsByPatient(patientId);

  const [treatmentMasterDataMap, setTreatmentMasterDataMap] = useState<
    Record<string, TreatmentMaster | null>
  >({});
  const [doctorDataMap, setDoctorDataMap] = useState<
    Record<string, Doctor | null>
  >({});
  // eslint-disable-next-line @typescript-eslint/no-unused-vars
  ...
  useEffect(() => {
    ...
    const uniqueTreatmentIds = [
      ...new Set(treatmentsList.map((t) => t.treatmentMasterId)),
    ];
    ...
    const fetchTreatmentMasters = async () => {
      ...
      try {
          ...
          uniqueTreatmentIds.map(async (id) => {
            try {
              const response = await fetch(
                `${process.env.NEXT_PUBLIC_API_BASE_URL}/api/treatmentmaster/${id}`,
                {
                  method: "GET",
                  headers: {
              ...
              if (!response.ok)
                throw new Error(`Treatment Master Not Found: ${id}`);
        ...
        setTreatmentMasterDataMap((prev) => ({
          ...prev,
          ...Object.fromEntries(
            treatmentResults.map(({ id, result }) => [id, result])
          ),
        }));
      } catch (error) {
    ...
    const fetchDoctors = async () => {
      ...
      try {
        ...
        setDoctorDataMap((prev) => ({
          ...prev,
          ...Object.fromEntries(
            doctorResults.map(({ id, result }) => [id, result])
          ),
        }));
      } catch (error) {
  ...
  return (
    <div className="w-full shadow-md rounded-lg">
      {treatmentListLoading ? (
        <p className="text-gray-500 text-center">Loading treatments...</p>
      ) : (
                  ...
                  treatmentsList.map((treatment) => (
                    <tr key={treatment._id} className="hover:bg-gray-50">
                      <td className="px-4 py-2 border text-center">
                        <Link
                          href={`/clinicTreatments/${treatment._id}`}
                      ...
                      <td className="px-4 py-2 border text-center">
                        {!loadingTreatmentMaster
                          ? treatmentMasterDataMap[treatment.treatmentMasterId]
                              ?.name || (
                              <span className="text-red-500 italic">
                                Treatment Name Not Found
                              </span>
                            )
                          : "Loading..."}
                      </td>
                      <td className="px-4 py-2 border text-center">
                        {!loadingDoctor
                          ? doctorDataMap[treatment.doctorId]?.name || (
                              <span className="text-red-500 italic">
                                Doctor Not Found
                              </span>
                            )
