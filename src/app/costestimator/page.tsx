...
const Page = () => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const treatments = await fetch(`${process.env.NEXT_PUBLIC_API_BASE_URL}/api/treatmentmaster`, {
                    method: "GET",
                    headers: {
    ...
    useEffect(() => {
        if (selectedData.selectedProcedure) {
            ...
            if (selectedProcedure?.specification && selectedProcedure.specification?.length > 0) {
    ...
    function calculateFinalCost() {
        if (current && current.specification) {
            ...
            const insuranceAdjustment =
                insurance.find((curr) => curr.name === selectedData.selectedInsurance)?.discount || 0;
            const discountAmount = (insuranceAdjustment / 100) * current.cost + priceAdjustment;
            ...
            >
                ...
                {cost > 0 && (
                    <div className="mt-6 p-4 bg-green-100 border-l-4 border-green-500 text-green-700 rounded-lg">
                        <p className="text-lg font-semibold">Estimated Cost: <span className="font-bold">${cost}</span></p>
                    </div>
                )}
            </form>
        </div>
